...
void main_levelInit(void *arg)
{
	...
	// scroll.w = 192;
	// scroll.h = 192;
	// animator = SDLX_AnimatorCreate(NULL, anim, 1, &scroll);
	...
	// ctx.buttons[2]->data = (void *)SDLX_AnimatorCreate(NULL, beam, 1, &beamrect);
	...
	SDLX_GUIElem_SetDefaultTarget(ctx.buttons[3]);
